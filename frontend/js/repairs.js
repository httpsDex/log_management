...
async function loadRepairHistory() {
  const res = await API.getRepairs();
  if (!res?.ok) return;
  _repairHistoryCache = res.data.filter(r => r.status === 'Released');
  document.getElementById('repairHistoryBody').innerHTML = _repairHistoryCache.length === 0
    ? emptyState('No released repairs yet', 12)
    : _repairHistoryCache.map(r => `<tr>
        <td class="td-mono">#${r.id}</td>
        <td class="td-name">${r.customer_name}</td>
        <td>${r.office}</td>
        <td>${r.item_name}</td>
